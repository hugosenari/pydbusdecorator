...
class <xsl:value-of select="dta:replace(string(@name), '^.+\.([^\.]+)$', '\1')"/>(object):
	...
	<xsl:value-of select="@name"/>
		...
		----------
		    ...
		    <xsl:value-of select="@type"/>, direction: <xsl:value-of select="@direction"/>,
		</xsl:for-each>
		</xsl:when>
		<xsl:when test="name(.) = 'property'"> property: <xsl:value-of select="@type"/>, <xsl:value-of select="@access"/></xsl:when>
	    </xsl:choose>
    </xsl:template>
</xsl:stylesheet>